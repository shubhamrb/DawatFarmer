...
public class FieldAdapter extends RecyclerView.Adapter<FieldAdapter.DashboardListViewHolder> {
    ...
    public FieldAdapter(Context mContext, FieldClickListener listener) {
    ...
    public DashboardListViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        root = LayoutInflater.from(mContext).inflate(R.layout.field_list_item, parent, false);
    ...
    public void onBindViewHolder(@NonNull DashboardListViewHolder holder, int position) {
        if (list.size() > 0) {
            ...
            if (model.getStatus().equals("1")) {
                holder.txt_field_count.setText("PRC Land " + model.getPrctotal() + " Acres");
                holder.txt_farm_size.setText(model.getTotal() + " Acres");
                holder.farm_card.setCardBackgroundColor(mContext.getResources().getColor(R.color.yellow_dim, null));
            } else {
                holder.txt_field_count.setText("No land  details");
                holder.txt_farm_size.setText("N/A");
                holder.farm_card.setCardBackgroundColor(mContext.getResources().getColor(R.color.cream_color, null));
            }
